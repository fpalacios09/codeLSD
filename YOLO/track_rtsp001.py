...
while True:
    ...
    # Pasar el frame a YOLO para realizar el seguimiento
    result = model.track(source=frame, **tracker_config)  # Aquí el frame es directamente pasado al modelo

    # Mostrar el polígono de la zona
    ...
    # Iterar sobre el generador de resultados
    for r in result:  # El resultado es un generador, por lo que iteramos sobre cada "frame" procesado
        # `r.boxes` es la información sobre las cajas detectadas
        for det in r.boxes:
    ...
    # Mostrar el total de vehículos únicos detectados
